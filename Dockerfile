...
ENV BROCOIN_VERSION 0.21.0
ENV BROCOIN_FILENAME brocoin-$BROCOIN_VERSION-x86_64-linux-gnu.tar.gz
ENV BROCOIN_URL https://brocoincore.org/bin/brocoin-core-$BROCOIN_VERSION/$BROCOIN_FILENAME
ENV BROCOIN_SHA256 da7766775e3f9c98d7a9145429f2be8297c2672fe5b118fd3dc2411fb48e0032
ENV BROCOIN_ASC_URL https://brocoincore.org/bin/brocoin-core-$BROCOIN_VERSION/SHA256SUMS.asc
ENV BROCOIN_PGP_KEY 01EA5486DE18A882D4C2684590C8019E36C2E964
RUN [ -n "$STANDALONE" ] || \
    (mkdir /opt/brocoin && cd /opt/brocoin \
    && wget -qO "$BROCOIN_FILENAME" "$BROCOIN_URL" \
    && echo "$BROCOIN_SHA256 $BROCOIN_FILENAME" | sha256sum -c - \
    && gpg --keyserver keyserver.ubuntu.com --recv-keys "$BROCOIN_PGP_KEY" \
    && wget -qO brocoin.asc "$BROCOIN_ASC_URL" \
    && gpg --verify brocoin.asc \
    && cat brocoin.asc | grep "$BROCOIN_FILENAME" | sha256sum -c - \
    && BD=brocoin-$BROCOIN_VERSION/bin \
    && tar -xzvf "$BROCOIN_FILENAME" $BD/brocoind $BD/brocoin-cli --strip-components=1 \
