...
[![build status](https://api.travis-ci.org/ElementsProject/lightningd-charge.svg)](https://travis-ci.org/ElementsProject/lightningd-charge)
[![npm release](https://img.shields.io/npm/v/lightningd-charge.svg)](https://www.npmjs.com/package/lightningd-charge)
[![docker release](https://img.shields.io/docker/pulls/shesek/lightningd-charge.svg)](https://hub.docker.com/r/shesek/lightningd-charge/)
[![MIT license](https://img.shields.io/github/license/elementsproject/lightningd-charge.svg)](https://github.com/ElementsProject/lightningd-charge/blob/master/LICENSE)
[![Pull Requests Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen.svg)](http://makeapullrequest.com)
[![IRC](https://img.shields.io/badge/chat-on%20freenode-brightgreen.svg)](https://webchat.freenode.net/?channels=lightningd-charge)
...
```bash
$ npm install -g lightningd-charge
...
$ docker run -it -u `id -u` -v `pwd`/data:/data -p 9735:9735 -p 9112:9112 \
             shesek/lightningd-charge --api-token mySecretToken
```

This will start `brocoind`, `lightningd` and `charged` and hook them up together.
...
```bash
$ docker run -it -e NETWORK=regtest -p 9112:9112 shesek/lightningd-charge --api-token mySecretToken
```
...
mount the brocoin data directory to `/etc/brocoin` (e.g. `-v $HOME/.brocoin:/etc/brocoin`).
...
[One-click deployment on Azure](https://github.com/NicolasDorier/lightningd-charge-azure)
...
Clients libraries are available for [JavaScript](https://github.com/ElementsProject/lightningd-charge-client-js)
and [PHP](https://github.com/ElementsProject/lightningd-charge-client-php).
...
- [Lightning Publisher](https://github.com/ElementsProject/wordpress-lightning-publisher): accept brocoin payments for content on WordPress blogs.
...
- [Lightning Jukebox](https://github.com/ElementsProject/lightning-jukebox): a lightning powered jukebox. Pay with Brocoin to choose your music.
...
or provide a `currency` and `amount` to be converted according to the current exchange rates (via brocoinaverage).
...
(see [`POST /invoice/:id/webhook`](https://github.com/ElementsProject/lightningd-charge#post-invoiceidwebhook)).
...
Requires `brocoind`, `brocoin-cli`, `lightningd`, `lightning-cli`
and [`jq`](https://stedolan.github.io/jq/download/) to be in your `PATH`.

```bash
$ git clone https://github.com/M0hitkumar/lightningd-charge.git
$ cd lightningd-charge
$ npm install
$ npm test
```

This will setup a temporary testing environment with a brocoind regtest node
