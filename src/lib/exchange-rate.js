...
const getRateCoingecko = currency =>
  request.get(`https://api.coingecko.com/api/v3/simple/price?ids=brocoin&vs_currencies=${enc(currency)}`)
    .proxy(RATE_PROXY)
    .then(res => res.body.brocoin[currency.toLowerCase()] || Promise.reject(`Unknown currency: ${currency}`))
