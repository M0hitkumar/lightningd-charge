...
(command -v brocoind && command -v brocoin-cli) > /dev/null || { echo >&2 "brocoind and brocoin-cli must be in PATH. to install, see https://brocoin.org/en/full-node#linux-instructions"; exit 1; }
...
alias btc="brocoin-cli --datadir=$BTC_DIR --rpcwallet=miner"
...
echo Setting up brocoind >&2
mkdir -p $BTC_DIR
cat >$BTC_DIR/brocoin.conf <<EOL
regtest=1
printtoconsole=0
fallbackfee=0.00001
...
brocoind -datadir=$BTC_DIR $BROCOIND_OPTS &

echo - Waiting for brocoind to warm up... > $dbgout
...
LN_OPTS="$LN_OPTS --network=regtest --brocoin-datadir=$BTC_DIR --log-level=debug --log-file=debug.log
